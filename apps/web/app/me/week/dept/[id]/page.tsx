...
export default async function DeptWeekPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const [r, allDepts] = await Promise.all([getDeptDashboard(id, "week"), getAllDepartments()]);
    ...
    return (
      <div className="page-content max-w-[1100px]">
        <div className="page-head">
          <div>
            <div className="page-title">Department week</div>
      ...
      <div className="page-head">
        <div>
          <div className="page-title" style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {r.stats.deptName} · Week
          </div>
          <div className="page-sub">{r.stats.rangeLabel}</div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/me/month/dept/${id}`} className="btn btn-ghost btn-sm">Month →</Link>
          <Link href="/me/week" className="btn btn-ghost btn-sm">← My week</Link>
        </div>
      </div>
