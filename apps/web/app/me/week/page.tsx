...
export default async function MyWeekPage() {
  ...
  const r = await getOrGenerateDashboard("week");
  ...
  return (
    <div className="page-content max-w-[1100px]">
      <div className="page-head">
        <div>
          <div className="page-title">My week · {me.name.split(/\s+/)[0]}</div>
          <div className="page-sub">{r.stats.rangeLabel} · {r.cached ? "cached" : "fresh"}</div>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/me/month" className="btn btn-ghost btn-sm">Month →</Link>
          <form action={refreshDashboard}>
            <input type="hidden" name="period" value="week" />
            <button type="submit" className="btn btn-ghost btn-sm" title="Regenerate this week's dashboard">
