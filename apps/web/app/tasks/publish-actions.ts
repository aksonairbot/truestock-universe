...
export async function publishNow(formData: FormData): Promise<void> {
  ...
  if (!isPrivileged(me)) throw new Error("Only admins and managers can publish.");
  ...
  // needs an image attached" is actionable; "publish failed" is not.
  if (!result.ok) throw new Error(result.error ?? "Publishing failed.");
}
 ...
 */
export async function resetPublishState(formData: FormData): Promise<void> {
  const taskId = ((formData.get("taskId") as string) ?? "").trim();
  ...
  const me = await getCurrentUser();
  if (!isPrivileged(me)) throw new Error("Only admins and managers can change publish state.");
  const task = await requireTaskAccess(taskId, me);
  ...
  if (markPublished) {
    if (!task.contentApprovedAt) throw new Error("This needs approval before it can be marked published.");
    if (manualUrl && !/^https?:\/\//i.test(manualUrl)) throw new Error("The live URL must start with http:// or https://");
